    ...
    object Matcher : TransformationMatcher {
        ...
        override fun match(state: MatchingState): TransformationMatch? {
            ...
            while (true) {
                ...
                if (MaxOrMinTransformation.Matcher
                        .match(currentState) != null
                ) break // do not take 'if' which is required for min/max matcher
        ...
        private fun matchOneTransformation(
            ...
            if (!state.inputVariable.hasUsages(condition) && (state.indexVariable == null || !state.indexVariable
                    .hasUsages(condition))
            ) return null
        ...
        private fun createFilterTransformation(
            ...
            if (!onlyFilterOrFilterNot) {
                if (conditionAsExpression is KtIsExpression
                    ...
                    && conditionAsExpression.leftHandSide
                        .isSimpleName(inputVariable.nameAsSafeName) // we cannot use isVariableReference here because expression can be non-physical
                ) {
